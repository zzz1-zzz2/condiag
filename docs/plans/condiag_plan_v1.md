...
Gate 1: Official Eval (attempt_1)
  ...
  缺失: raw eval log 未统一保存

Gate 2: Failure Witness Builder          ← 新增
  输入: Gate 1 raw eval log（优先）或重新运行 Docker（fallback）
  执行: 从 raw log 解析 failure output
  产出: failure_witness.json
  已实现: TODO

Gate 3: Retry Eligibility
  输入: Gate 1 结果, Gate 2 结果, intervention_report
  执行: 筛选 eligible cases
  产出: first_failed_candidate_matrix.csv
  已实现: TODO

Gate 4: Packet Generation
  输入: failure_witness, diagnosis, evidence, api_navigation
  执行: context_packet_builder v2
  产出: context_packet.md (per baseline type)
  已实现: PARTIAL（v1 框架有，缺 4 个 section）

Gate 5: Host-Agent Retry
  输入: context_packet + original issue + failure_witness
  执行: mini-SWE attempt_2 (tool loop)
  产出: attempt_2/patch.diff (workspace_git_diff)
  已实现: DONE (MinisweRetryInjectionAdapter)

Gate 6: Official Eval (attempt_2)
  输入: attempt_2/patch.diff
  执行: 同 Gate 1
  产出: attempt_2_resolved
  已实现: DONE

Gate 7: Rescue Matrix
  输入: Gate 1 + Gate 6 结果, cost 数据
  执行: 汇总统计
  产出: retry_rescue_matrix.csv + packet_to_patch_trace.csv
  已实现: PARTIAL（10 条记录）
```
...
验收标准：
- 19 个 instance 都有 eval_status = EVALUATED
