...
def main():
    ...
    for i, iid in enumerate(first_failed):
        ...
        try:
            # Build witness — if no log, create diagnostic from official_eval
            if raw_log:
                ...
                if apply_error:
                    ...
                    # Fall back to builder which returns no-witness
                    witness = build_failure_witness(
                        instance_id=iid,
                        eval_log_path=None,
                        method_version="v2.1",
            ...
            print(f"  [{i+1:2d}/{len(first_failed)}] {iid[:40]:40s} parser={witness.parser_name:20s} stage={witness.failure_stage:35s} quality={witness.quality:20s} eligible={witness.eligible_for_condiag} {size_info}")
