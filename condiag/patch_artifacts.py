...
def collect_agent_submission(
    ...
    if len(sources) >= 2:
        # Compare each source's SHA — if any differ significantly, flag
        shas = {sha256_short(s) for s in sources}
        sub.consistency_status = "mismatch" if len(shas) > 1 else "consistent"
    elif len(sources) == 1:
